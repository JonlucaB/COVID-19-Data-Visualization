class State {
 ...
 State(String _name, String _file_Name, String _image_Name, int _image_Length, int _image_Height, float _lat_Start, float _lat_End, float _lon_Start, float _lon_End, float mcases, float mdeaths, float mpopulation) {
   ...
   image_xx = ((image_Length * height)/image_Height);
   image_Print_xx = (width/2) - (image_xx/2);
   
   scatterX_StartX = int ((.01 * image_xx) + image_Print_xx);
   scatterX_Y = int (.93 * height);
   scatterX_EndX = int ((.98 * image_xx) + image_Print_xx);
   
   scatterY_X = int (.03 * image_xx) + image_Print_xx;
   scatterY_StartY = int (.95 * height);
   scatterY_EndY = int (.03 * height);
 ...
 void display_Image() {
   background(255);
   image(img, image_Print_xx, 0, image_xx, height);
 }
 ...
 void display_Scatter() {
    ...
    textSize(int (width * .01));
    fill(0);
    
    text("% Voted Republican", width/2, height * 96);
    text("% Cases / Population", scatterX_StartX, height * .014);
    //x vals
    for(int r = 0; r < 100; r += 5) {
      text(r, map(r, 0, 100, scatterY_X, scatterX_EndX) + (width * .0013), scatterX_Y * 1.02);
    }
    //y vals
    int compensate_for_Y = int (height * .01);
    for(int c = 0; c < 15; c += 5) {
      text(c, scatterY_X - (width * 0.015), map(c, 0, 10, scatterX_Y - compensate_for_Y, compensate_for_Y + scatterY_EndY));
    }
    
    text("% Voted Republican", width/2, height * .97);
    text("% Cases / Population", scatterX_StartX, height * .014);
