...
public void setup() {
  ...
  Texas = new State("Texas", "./Resources/Texas_Set.csv", "./Resources/Texas.jpg", 1678, 1447, 25.8f, 36.5f, 93.6f, 106.3f, 162306, 2811, 4525519); 
  Colorado = new State("Colorado", "./Resources/Colorado_Set.csv", "./Resources/Colorado.jpg", 320, 232, 37, 41, 102, 109, 31753, 628, 1010420);
  Georgia = new State("Georgia","./Resources/Georgia_Set.csv","./Resources/Georgia.jpg",298,342,30.3f,35,80.5f,85.76f, 19878, 452, 678467); 
  selected_State = Texas;
  scaler = new Drag_Circle_Scale(width*.025f, height * .3f, 20, width*.045f, width*.15f, 0, selected_State.max_Cases, selected_State.max_Cases, selected_State.max_Deaths, selected_State.max_Population);
  windowResizable(false);
  instructX = PApplet.parseInt(width*.025f);
  instructY = PApplet.parseInt(height * .8f);
  instructW = 200;
  instructH = 100;
}
...
class County {
    ...
    public float getSize() {
      ...
      //option 3 = cases 
      return map(cases/population, 0, 1, 20, width * .25f);
    }
    ...
    public void Map_Display(boolean option, boolean circle_selected) {
      ...
      if(hold_info_box || (mouse_over_circle(1) && !circle_selected)) {
        fill(255);
        stroke(0);
        float rectX = width * .06f;
        float rectY = height * .8f;
        float rectW = width * .1f;
        float rectH = height * .14f;
...
class Drag_Circle_Scale {
  ...
  public float display() {
   ...
   if(filter == 1) {
    ...
    if(greater_than) {
       text('>', left_Bound + 120, y + d * 1.55f); 
    } else {
      text('<', left_Bound + 120, y + d * 1.55f);
    }
    
    text(PApplet.parseInt (map(x, left_Bound, right_Bound, scale_Left, scale_Right)), left_Bound - 20, y - 50);
...
class State {
 ...
 State(String _name, String _file_Name, String _image_Name, int _image_Length, int _image_Height, float _lat_Start, float _lat_End, float _lon_Start, float _lon_End, float mcases, float mdeaths, float mpopulation) {
   ...
   scatterX_Y = PApplet.parseInt (.93f * height);
   scatterX_EndX = PApplet.parseInt ((.98f * image_xx) + image_Print_xx);
   
   scatterY_X = PApplet.parseInt (.03f * image_xx) + image_Print_xx;
   scatterY_StartY = PApplet.parseInt (.95f * height);
   scatterY_EndY = PApplet.parseInt (.03f * height);
 ...
 public void display_Scatter() {
    ...
    textSize(PApplet.parseInt (width * .01f));
    fill(0);
    
    text("% Voted Republican", width/2, height * 96);
    text("% Cases / Population", scatterX_StartX, height * .014f);
    //x vals
    for(int r = 0; r < 100; r += 5) {
      text(r, map(r, 0, 100, scatterY_X, scatterX_EndX) + (width * .0013f), scatterX_Y * 1.02f);
    }
    //y vals
    int compensate_for_Y = PApplet.parseInt (height * .01f);
    for(int c = 0; c < 15; c += 5) {
      text(c, scatterY_X - (width * 0.015f), map(c, 0, 10, scatterX_Y - compensate_for_Y, compensate_for_Y + scatterY_EndY));
    }
    
    text("% Voted Republican", width/2, height * .97f);
    text("% Cases / Population", scatterX_StartX, height * .014f);
...
class User_Settings {
  ...
  public boolean Display() {
    ...
    stroke(0);
    strokeWeight(2);
    rectX = PApplet.parseInt (width * .04f);
    rectY = PApplet.parseInt (height * .15f);   
    rectH = PApplet.parseInt (height * .035f);
    
    rect(rectX, rectY, rectX, rectH);
    
    fill(0);
    if(option_selected) {
      text("2016", rectX * 1.1f, rectY * 1.05f, rectX, rectH);
      return true;
    } else {
      text("2020", rectX * 1.1f, rectY * 1.05f, rectX, rectH);
      return false;
    }
  }
